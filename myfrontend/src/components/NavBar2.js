...
function NavBar2({type,credits,islogged,user3, user2, logout,setIsadmin, userid,isadmin}) {
    ...
    const opensavedads=()=>{
        ...
        const openHome=()=>{
          ...
          const openProfile=()=>{
            console.log(type)
            if(type=="Mechanic"){
              console.log('/mechanicprofile/'+userid)
              history.push('/mechanicprofile/'+userid);
    ...
    return (
        <div>
            ...
            <Navbar className="color-nav" variant="dark">
                  ...
                  {islogged==="true" 
                  ? type=="Mechanic"?null: <Nav style={{}} className="mr-auto">
                    <Nav.Link style={{color: 'white'}} href={'/mypurchases/'+userid}>My Purchases</Nav.Link>
                    </Nav>:null}
                  <Nav className="mr-auto" >
                    {islogged==="true"&&credits>0?<Nav.Link style={{color: 'white', paddingright:'0%', backgroundColor:'#0d3f67'}} >Credits: {credits}</Nav.Link>:null}
                    {islogged==="true"
                    ...
                    ?<NavDropdown style={{color: 'white', fontWeight: 'bold'}} title={user2} id="nav-dropdown">
                      {type=="User"?<NavDropdown.Item eventKey="4.1" className="nav-items" href={'/mycart'}>Cart</NavDropdown.Item>:null}
                  ...
                  {isadmin==="true"?<Nav style={{position:'absolute', left:'90%'}} className="mr-auto">
                    ...
                    <NavDropdown.Item eventKey="4.1" className="nav-items" href={'/purchases'}>All Purchases</NavDropdown.Item>
