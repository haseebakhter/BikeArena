...
function Bannedusers({isadmin}) {
    ...
    if(isadmin==="true"){
        return (
            <div>
                <h3 style={{borderLeft: '6px solid #1423A4', backgroundColor: '#E52D27', color: 'white'}}>{results.length} Banned Users</h3>
                {results.map(name=>(
                    ...
                    <img width="100" height="100" src={'/content/'+name.propic} ></img>
                            ...
                            <button onClick={()=>{handleUnBan(name.email)}} style={{float:'right'}} type="button" class="btn btn-danger">UnBan User As Admin</button>
